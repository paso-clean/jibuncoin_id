...
if(!isset($_COOKIE['xsystem_' . XSYSTEM_APP . '_session'])){
  ...
  if($user = $userIns->get_user_by_session($_COOKIE['xsystem_' . XSYSTEM_APP . '_session'])){
    ...
    if($userIns->is_secure_lock($session_code)){
        setcookie("xsystem_" . XSYSTEM_APP . "_session",$session_code,time()+60*60*24*30,APP_URI . '/' . XSYSTEM_APP);
        $redirect = XSYSTEM_APP_URL . 'secure/onetime/' . $session_code;
        header("Location: $redirect");
        exit;
    }
  }else{
      setcookie("xsystem_" . XSYSTEM_APP . "_session", "", time() - 30,APP_URI . '/' . XSYSTEM_APP);
    $redirect = XSYSTEM_APP_URL . 'login/';
    header("Location: $redirect");
    exit;
...
<?php if($i == $init_num){ ?>
    <div id="position-<?php echo $i; ?>-btn" class="position-btn pointer btn-slide active" data-panel=<?php echo $i; ?>><?php echo $apps[$i]['title']; ?></div>
<?php }else{ ?>
    <div id="position-<?php echo $i; ?>-btn" class="position-btn pointer btn-slide"  data-panel=<?php echo $i; ?>><?php echo $apps[$i]['title']; ?></div>
<?php } ?>
<?php } ?>
</div>
</div>
...
<div class="login-user btn-ajax" data-url="<?php echo XSYSTEM_APP_URL; ?>ajax/user/profile/"><img id="header-user-icon" class="reload_class_<?php echo $user['objects']['user_img'][0]['object_code']; ?>" src="<?php echo XSYSTEM_IMG_URL; ?>user/thum/<?php echo $user['objects']['user_img'][0]['object']; ?>" ></div>
</div>
