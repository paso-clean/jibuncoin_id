...
// 				$this->_create_secure_session($session_data['session_code'],$user['secure']);
	...
	function logout($session_code){
	    ...
	    if($session_data = $results->fetch_assoc()){
	        ...
	        try {
	            $query = "DELETE FROM " . XSYSTEM_APP . "_sessions WHERE target_code = '" . $session_data['target_code'] . "'";
	            Db::$db->query($query);
	            
	            $query = "DELETE FROM " . XSYSTEM_APP . "_sessions WHERE target_code = '" . $session_code . "'";
	            Db::$db->query($query);
	        } catch( Exception $e ){
	            Db::$db->rollback();
	            $security_sessions = array();
	        }
	        Db::$db->commit();
	    }
	}
